	...
	class Commands : BaseCommandModule
	{
		...
		public async Task ShowFormPrivate(CommandContext ctx)
		{
			...
			await Members[ctx.User.Id].SendMessageAsync(
					...
					{
						Color = new DiscordColor("#B388FD"),
						Description = $"Используйте команду **-fr** чтобы найти случайного человека или **-fbt** чтобы найти человека по тегам. Команды можете вводить как на сервере чтобы все видели кого вам кидает, так и прямо сюда."
					});
		}
